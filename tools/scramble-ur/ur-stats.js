...
(function () {
  ...
  function trendHtml(t) {
    if (!isFinite(t)) return '<div class="ur-trend ur-trend--na">趋势 待累积</div>';
    var p = Math.abs(t) * 100;
    if (p < TREND_FLAT) return '<div class="ur-trend ur-trend--flat">趋势 → 持平 ' + p.toFixed(1) + '%</div>';
    if (t < 0) return '<div class="ur-trend ur-trend--down">趋势 ↓ 进步 ' + p.toFixed(1) + '%</div>';
    return '<div class="ur-trend ur-trend--up">趋势 ↑ 变慢 ' + p.toFixed(1) + '%</div>';
  }
